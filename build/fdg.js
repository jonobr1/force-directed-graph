...
`;
  ...
  // src/index.js
  var ForceDirectedGraph =  window.ForceDirectedGraph = class extends import_three4.Group {
    constructor(renderer, data) {
      super();
      const registry = new Registry(data.nodes);
      const size = getPotSize(Math.max(data.nodes.length, data.links.length));
      const gpgpu = new import_GPUComputationRenderer.GPUComputationRenderer(size, size, renderer);
      const uniforms = {
        decay: { value: 1 },
        alpha: { value: 1 },
        is2D: { value: false },
        ...
        pointColor: { value: new import_three4.Color(1, 1, 1) },
        linkColor: { value: new import_three4.Color(1, 1, 1) },
        opacity: { value: 1 }
      };
      const textures = {
      ...
      variables.positions.wrapS = variables.positions.wrapT = import_three4.RepeatWrapping;
      variables.velocities.wrapS = variables.velocities.wrapT = import_three4.RepeatWrapping;
